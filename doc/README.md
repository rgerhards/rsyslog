...
### Building via autotools targets

After running the usual autotools bootstrap (`./autogen.sh && ./configure`), you can invoke Sphinx directly from the top-level build tree:

-   `make doc-html` or simply `make html` for the standard multi-page HTML output
-   `make doc-singlehtml` for the minimal single-page HTML build
-   `make doc-json`/`make doc-alljson` for JSON output consumed by downstream tooling
-   `make doc-clean` to remove the `doc/build` directory generated by the Sphinx builders

The doc-specific make targets honour the `SPHINXBUILD`, `SPHINXOPTS`, and `O` variables, so you can override them on the command line if needed.
