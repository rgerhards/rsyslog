...
- uploads the review package, evaluator output, and normalized summary
...
## Replacing Deterministic Evaluation Later

The workflow now uses a deterministic evaluator because these three checks are
better expressed as repository rules than model judgments.

If a future extension wants to add an LLM or external evaluator again, keep
`scripts/build_repo_policy_focus_input.py` and
`scripts/score_repo_policy_focus.py` stable and replace only the evaluator step
in `.github/workflows/repo-policy-focus-review.yml`.
