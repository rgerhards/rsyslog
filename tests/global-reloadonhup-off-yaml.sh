#!/bin/bash
# Verify native YAML reloadOnHUP=off keeps the legacy omfile reopen hook. A
# message is sent through an imtcp-bound YAML ruleset, the file is renamed,
# and the post-HUP message must be written to a newly opened original path.
. ${srcdir:=.}/diag.sh init
require_yaml_support
require_plugin imtcp
generate_conf --yaml-only
sed -i '/debug.abortOnProgramError:/a\  config.reloadOnHUP: "off"' "${TESTCONF_NM}.yaml"
