...
# peers. A final shrink is rejected and all three established sessions remain
# usable. socketBacklog is explicit so changing maxSessions does not also imply
# an unprepared listen-backlog change.
. ${srcdir:=.}/diag.sh init
require_plugin imtcp
generate_conf
add_conf 'global(config.reloadOnHUP="on")'
add_conf 'module(load="../plugins/imtcp/.libs/imtcp" maxSessions="1")'
add_conf 'input(type="imtcp" address="127.0.0.1" port="0" listenPortFileName="'$RSYSLOG_DYNNAME'.tcpflood_port" name="sessions" socketBacklog="32" ruleset="main")'
...
sed 's/socketBacklog="32"/maxSessions="3" socketBacklog="32"/' "$CONF_FILE.module-grow" >"$CONF_FILE.input-grow"
